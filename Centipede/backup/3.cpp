...
void drawCrHead(sf::RenderWindow& window,  sf::Sprite& crHeadSprite);
void drawCrBody(sf::RenderWindow& window,sf::Clock& crBodyClock,  sf::Sprite& crBodySprite);
void petrol();
void motion(sf::Clock& crBodyClock);
...
{	
	...
	petrol();//petrol function will set the postion of centipede and also manages motion
	//////////////////////////////////////////////////////////////////////////
	...
	while(window.isOpen()) {
		...
		motion(crBodyClock);
		drawCrHead(window , crHeadSprite);
		drawCrBody(window,crBodyClock , crBodySprite);
...
void petrol()
{
//initializing the postion of head
int startP=rand()%20;
gameGrid[startP][0]=6;
 ...
 {	
		...
		{
		 int length=++i+10;
			for(i; i< length;i++) gameGrid[i][0]=7;		
			 break;
		}	
 }
}
...
void drawCrHead(sf::RenderWindow& window, sf::Sprite& crHeadSprite) {
	...
	{		
		if(gameGrid[i][0]==6)
		{	
		crHeadSprite.setPosition(i*boxPixelsX, 32);	
...
void drawCrBody(sf::RenderWindow& window,sf::Clock& crBodyClock,  sf::Sprite& crBodySprite) {
	...
	{		
		...
		crBodySprite.setPosition(i*boxPixelsX, 32);	
...
for(int i=0;count!=0 && i<gameColumns; i++)
 {	
		if(  gameGrid[i][0]==6||gameGrid[i][0]==7)
		{
			count=i;
			do
			{
			if (crBodyClock.getElapsedTime().asMilliseconds() < 100)
		return;
		crBodyClock.restart();
 ...
 /*
 for(int i=0;count!=30 && i<gameColumns; i++)
 {	
		if(  gameGrid[i][0]==7 && gameGrid[i+1][0]==0)
		{
		printGameGrid();
			count=i;
			do
